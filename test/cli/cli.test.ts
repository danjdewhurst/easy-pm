...
import { setupTestServer, teardownTestServer, resetTestDb, getTestBaseUrl, getTestToken } from "../server/helpers.ts";
...
function cliAuth(...args: string[]): Promise<{ stdout: string; stderr: string; exitCode: number }> {
...
describe("CLI", () => {
  ...
  test("project create and list", async () => {
    const create = await cliAuth("project", "create", "--name", "Test Project", "--format", "json");
  ...
  test("project CRUD flow", async () => {
    ...
    // Get
    const get = await cliAuth("project", "get", "--id", "1", "--format", "json");
    expect(JSON.parse(get.stdout).data.name).toBe("P1");

    // Update
    const update = await cliAuth("project", "update", "--id", "1", "--name", "Updated", "--format", "json");
    expect(JSON.parse(update.stdout).data.name).toBe("Updated");

    // Delete
    const del = await cliAuth("project", "delete", "--id", "1", "--format", "json");
    expect(JSON.parse(del.stdout).ok).toBe(true);
  });

  test("search command", async () => {
    await cliAuth("project", "create", "--name", "P1", "--format", "json");
    await cliAuth("board", "create", "--project-id", "1", "--name", "B1", "--format", "json");
    await cliAuth("column", "create", "--board-id", "1", "--name", "To Do", "--format", "json");
    await cliAuth("card", "create", "--column-id", "1", "--title", "Fix authentication bug", "--format", "json");

    const result = await cliAuth("search", "authentication", "--format", "json");
    expect(result.exitCode).toBe(0);
  ...
  test("returns error without token for protected routes", async () => {
    const result = await cli("--api-url", getTestBaseUrl(), "project", "list", "--format", "json");

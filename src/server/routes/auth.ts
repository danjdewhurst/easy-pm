...
import type { User, PublicUser, Session, AuthResponse } from "../../shared/types.ts";
import { SESSION_EXPIRY_DAYS } from "../../shared/constants.ts";
import { validateEmail, validatePassword, parseJsonBody } from "../../shared/validate.ts";
...
function createSession(userId: number): Session {
  ...
  return db.query("SELECT * FROM sessions WHERE token = ?").get(token) as Session;
}

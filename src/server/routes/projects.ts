...
export function listProjects(): Response {
  const db = getDb();
  const projects = db.query("SELECT * FROM projects ORDER BY id").all() as Project[];
...
export function getProject(_req: Request, params: Record<string, string>): Response {
  const db = getDb();
  const project = db.query("SELECT * FROM projects WHERE id = ?").get(params.id!) as Project | null;
  if (!project) throw new NotFoundError("Project", params.id);
  return jsonResponse(project);
}

export async function updateProject(req: Request, params: Record<string, string>): Promise<Response> {
  const db = getDb();
  const existing = db.query("SELECT * FROM projects WHERE id = ?").get(params.id!) as Project | null;
  ...
  const updated = db
    .query(`UPDATE projects SET ${sets}, updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now') WHERE id = ? RETURNING *`)
    .get(...(values as [string])) as Project;
  return jsonResponse(updated);
}

export function deleteProject(_req: Request, params: Record<string, string>): Response {
  const db = getDb();
  const existing = db.query("SELECT * FROM projects WHERE id = ?").get(params.id!) as Project | null;
  if (!existing) throw new NotFoundError("Project", params.id);

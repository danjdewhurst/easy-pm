...
export function ColumnComponent({ column, labels, onUpdate }: ColumnProps) {
  ...
  return (
    <div className="flex-shrink-0 w-72 bg-slate-800/50 rounded-xl border border-slate-700/50 flex flex-col max-h-full">
      {/* Column header */}
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-700/30">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-slate-300">{column.name}</h3>
        ...
        {showNewCard ? (
          <form onSubmit={handleCreateCard}>
            <input
              ...
              className="w-full px-2.5 py-1.5 text-sm bg-slate-900 border border-slate-600 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
            />
          </form>
        ) : (
          <button
            onClick={() => setShowNewCard(true)}
            className="w-full text-left px-2.5 py-1.5 text-sm text-slate-500 hover:text-slate-300 rounded-lg hover:bg-slate-700/30 transition-colors"
          >
            + Add card
          </button>
        )}
